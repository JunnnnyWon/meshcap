...
<p align="center">
  Meshy · Tripo로 뽑은 메시의 구멍을 찾아 메운 다음,<br />
...
생성 모델은 미리보기에서 멀쩡해 보인다. 슬라이서에 넣는 순간 겨드랑이, 머리카락 사이, 바닥이 뚫려 있다. 슬라이서는 법선으로 안팎을 보기 때문에 열린 메시는 속을 채우지 못한다.

GLB · STL · OBJ · PLY를 브라우저에 놓으면 구멍을 나눠 메우고, 밀폐·다양체·법선을 검사한 뒤 STL/GLB를 내려준다. 원본 파일은 서버로 안 올라간다. 기본은 브라우저에서 돌리고, 큰 모델만 좌표와 인덱스를 연산 서버로 넘길 수 있다.

<p align="center">
  <img src="docs/screenshots/demo.gif" width="920" alt="물결 개구부 튜브를 메우기 전과 후" />
</p>

<p align="center"><sub>예제 「물결 개구부 튜브」. 비평면 테두리를 Liepa 삼각화와 바닥 받침으로 닫음. 65점 → 100점.</sub></p>

## 하는 일

파일을 넣으면 대략 이 순서로 간다.

1. 좌표는 같은데 인덱스만 다른 정점을 합친다. UV 이음매를 구멍으로 오인하는 일을 여기서 막는다.
2. 짝이 없는 half-edge로 테두리를 잇는다. 면이 셋 이상 모인 자리에서도 순회가 끊기지 않게.
3. 면의 감는 방향을 맞춘 다음 구멍을 찾는다. 뒤집힌 면을 구멍으로 세지 않기 위해서다.
4. 구멍마다 크기·평면성·방향을 보고 메우는 방법을 고른다.
5. 밀폐, 다양체, 법선, 관통을 100점으로 채점한다.
6. STL 또는 GLB로 내보낸다.
...
합성 예제만이 아니다. 서비스에서 받은 캐릭터 STL을 브라우저에서 돌렸다.
...
<table>
  <tr>
    <td align="center" width="50%">
      <img src="docs/screenshots/09-meshy.png" alt="Meshy 캐릭터 309만 삼각형, 96점" />
      <br /><sub>meshy.stl · 309만 삼각형 · 94 → 96</sub>
    </td>
    <td align="center" width="50%">
      <img src="docs/screenshots/10-tripo.png" alt="Tripo 캐릭터 190만 삼각형, 99점" />
      <br /><sub>Tripo.stl · 190만 삼각형 · 97 → 99</sub>
    </td>
  </tr>
</table>

100점이 아닌 이유를 숨기지는 않았다. 구멍을 메우면 이미 면 셋이 붙어 있던 에지에 면이 하나 더 붙을 수 있다. 표면을 닫는 일과 다양체로 만드는 일이 가끔 충돌하고, 그래서 채점에서 둘을 갈라 두었다.

## 파이프라인

코어는 three.js를 안 쓴다. 순수 TypeScript라 웹 워커, Node 벤치, 연산 서버가 같은 코드를 돌린다. 화면에 찍힌 숫자와 서버가 준 숫자가 다를 일이 없다.
...
| 02 용접 | 같은 좌표 정점을 공간 해시로 합친다. UV 이음매 오탐을 여기서 없앤다 |
...
| 07 Cap | 삼각형 / 부채꼴 / 평면+earcut / Liepa DP / 바닥 받침. 틈이 줄어들 때까지 반복 |
...
「한 면만 접한 에지」로 테두리를 정하면 실제 모델에서 자주 끊긴다. 면 셋이 한 에지를 공유하면 그 자리는 경계가 아닌데, 따라가던 순회가 거기서 멈춘다. 위 Meshy 캐릭터는 비다양체 에지 93개 때문에 경계 정점 178개 중 117개의 차수가 어긋났고, 테두리가 사슬로만 잡혀 하나도 못 메웠다.
...
| 정점 3개 | 삼각형 하나 | 그걸로 닫힘 |
| 아래를 향한 큰 개구부 | 바닥 받침 | 베드에 평평하게 닿아야 첫 층이 안 뜸 |
...
좌표와 인덱스만 받아 같은 파이프라인을 돌리는 연산 서버도 같이 올려 두었다. 코어를 three.js에서 떼 둔 덕에 서버는 같은 파일을 그대로 불러 쓴다. `bench/api-check.ts`가 좌표 단위까지 맞춰 본다.
...
그래서 자동 모드는 속도 때문이 아니라 **이 기기가 버티느냐**로 서버를 고른다. 50만 이하 항상 브라우저, 400만 초과 항상 서버, 그 사이는 메모리와 코어 수를 본다. 화면에서 브라우저로 고정할 수 있고, 어디서 돌렸는지는 매번 보여 준다.
...
server/index.ts     연산 서버. src/core를 그대로 불러 씀
```
